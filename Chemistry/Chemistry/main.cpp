...
ll choose [200][200];
map <ll, map <int, ll>> dp; //first: number of cups, second: number of minutes;
...
ll C (int a, int b)
{
    ...
    return choose [a][b] = C (a - 1, b - 1) + C (a - 1, b);
}
...
ll solve (ll cups, int minutes)
{
    if (cups == 1)
    {
        return 0;
    }
    if (dp [cups][minutes] != 0)
    {
        return dp [cups][minutes];
    }
    if (minutes == 1)
    {
        int x = 0;
        ...
        return dp [cups][minutes] = x;
    }
    ll result = INFLL;
    for (int x = 1; x < 200; x++)
    {
        if (maxi [x] < 0) {break;}
        ll temp = solve ((ll)(ceil(double(cups)/(maxi [x] + 1))), minutes - 1);
        ll res = max ((ll)x, temp);
        result = min (res, result);
    }
    return dp [cups][minutes] = result;
}
