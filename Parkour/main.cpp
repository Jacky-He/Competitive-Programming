...
int x, y, h, v, t;
bool withinbound (pair <int, int> p)
{
    return x <= p.first && p.first <= x + h && y <= p.second && p.second <= y + v;
}
...
int main ()
{
    ...
    int cnt = 0;
    while (!withinbound(curr))
    {
        int xdiff = x - curr.first;
        int ydiff = y - curr.second;
        if (xdiff == ydiff)
        {
            if (h > v)
            {
